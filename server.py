...
def one_client(cl):
    while True:
        ...
        if data_obj.typ == "exit":
            ...
            cl.close()
            break
        service_client(data_obj.typ, data_obj.content, cl)
...
def service_client(function_type, content, client):
    ...
    if function_type == "register":
        user_registration_service(content.username, content.password)
...
def user_registration_service(username, password):
    set_user_login.add(username)
    dic_user_pwd[username] = password
